...
public:
     ...
     */
    ThreadData(size_t time_series_length)
        : length_(10000), timeseries_(time_series_length)
    {
        n_outputs_ = 10;
        srand(0);
...
void* timeseries_to_output(void* void_ptr)
{
    ...
    real_time_tools::ThreadsafeTimeseries<Type>& timeseries =
        thread_data.data_->timeseries_;
...
bool test_threadsafe_timeseries_history(bool slow)
{
    ...
    for (size_t i = 0; i < data.output_indices_.size(); i++)
    {
        ...
        threads[i].create_realtime_thread(&timeseries_to_output,
                                          &output_data[i]);
    }
